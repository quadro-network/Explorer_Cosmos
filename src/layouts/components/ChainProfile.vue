...
<template>
  <div class="dropdown">
    <label tabindex="0" class="flex items-center">
      <div class="p-1 relative mr-3 cursor-pointer">
        ...
        <div
          ...
          :class="{
            'bg-grey-950': baseStore.connected,
      ...
      <div class="flex-1 w-0">
        <div
          :key="
            ...
            ''
          "
          class="capitalize whitespace-nowrap text-base font-semibold text-grey-100 dark:text-grey-200 hidden md:!block"
        >
          {{
            baseStore.latest?.block?.header?.height
              ? `#${baseStore.latest.block.header.height}`
        ...
        <div class="text-xs text-customGreen whitespace-nowrap hidden md:!block">
    ...
    <div
      tabindex="0"
      class="border dropdown-content -left-6 w-80 menu shadow bg-customDark rounded-box overflow-auto"
    >
      <!-- rest -->
      <div class="px-4 py-2 text-sm text-grey-100" v-if="chainStore.current?.endpoints?.rest">
        Rest Endpoint
      </div>
      <div
        v-for="(item, index) in chainStore.current?.endpoints?.rest"
        class="px-4 py-2 w-full text-grey-100 hover:bg-grey-950 backdrop-blur-md cursor-pointer"
        :key="index"
        @click="changeEndpoint(item)"
      >
        <div class="flex flex-col">
          <div class="flex items-center justify-between w-full">
            <div class="text-grey-100 dark:text-grey-200 capitalize">
              {{ item.provider }}
            </div>
            <span
          ...
          <div class="text-grey-100 text-xs whitespace-nowrap">
            {{ item.address }}
          </div>
        </div>
      </div>
