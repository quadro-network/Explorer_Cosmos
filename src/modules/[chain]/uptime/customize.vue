...
function color(v: string) {
  if(v) {
    const n = Number(v)
    if(n < 10) return " badge-base-content"
...
<template>
  <div>
    <div class="overflow-x-auto w-full">
      <div class="lg:!flex lg:!items-center lg:!justify-between #saved p-5">
        <div class="min-w-0 flex-1">
          ...
          <div class="mt-1 flex flex-col sm:!mt-0 sm:!flex-row sm:!flex-wrap sm:!space-x-6">
            <div class="mt-2 flex items-center text-sm text-grey-500">
              <svg class="mr-1.5 h-5 w-5 flex-shrink-0 text-grey-400" viewBox="0 0 20 20" fill="currentColor"
                aria-hidden="true">
      ...
      <table class="table table-compact w-full">
        ...
        <tbody>
          <tr v-for="(v, i) in list" class="hover hover:customChocko">
            ...
            <td class=""><RouterLink :to="`/${v.chainName}/uptime/#blocks`" class="btn btn-xs  #sav1">{{ $t('module.blocks') }}</RouterLink></td>
          </tr>
        </tbody>
      </table>
    </div>
    ...
    <div class="text-center">
      <label for="add-validator" class="btn  #sav1 mt-5">{{ $t('uptime.add_validators') }}</label>
    </div>
    ...
    <div class="modal">
      <div class="modal-box relative">
        ...
        <div class="modal-action">
          <label class="btn  #sav1" @click="add">{{ $t('uptime.add') }}</label>
        </div>
      </div>
    </div>
    <div class="h-6"></div>
