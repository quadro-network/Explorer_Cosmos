...
<template>
 ...
 class="tabs tabs-boxed  mb-4">
      <a class="tab text-grey-100 capitalize" :class="{ 'tab-active': tab === '3' }" @click="changeTab('3')">{{
        $t('uptime.overall') }}</a>
      <a class="tab text-grey-100 capitalize" :class="{ 'tab-active': tab === '2' }" @click="changeTab('2')">{{
        $t('module.blocks') }}</a>
      <RouterLink :to="`/${chain}/uptime/customize`" >
        <a class="tab text-grey-100 capitalize">{{ $t('uptime.customize') }}</a>
      </RouterLink>
    </div>
    <div class="#saved px-5 pt-5">
      <div style="display: none"
 class="flex items-center gap-x-4">
        <input type="text" v-model="keyword" placeholder="Keywords to filter validators"
          class="input input-sm w-full flex-1 border border-grey-200 dark:border-grey-600" />
      ...
      <div :class="tab === '2' ? '' : 'hidden'">
        <div class="flex flex-row flex-wrap gap-x-4 mt-4 justify-center">
          <div v-for="(unit, i) in grid" :key="i">
            <div class="flex justify-between py-0 w-[248px]">
              ...
              <div v-else class="badge badge-sm bg-transparent text-customGreen border-0 font-bold">
                {{ unit?.missed_blocks_counter }}
              </div>
            </div>
        ...
        <div class="mt-5 flex bg-customDark border text-customGreen justify-center  gap-2">
      ...
      <div :class="tab === '3' ? '' : 'hidden'" class="overflow-x-auto">
        <table class="table table-compact w-full mt-5">
          <thead class="capitalize  bg-customDark border text-customGreen">
            <tr>
          ...
          <tr v-for="(v, i) in grid" class="hover">
            ...
            <td class="text-right">
              <span :class="v.uptime && v.uptime > 0.95 ? 'text-grey-50' : 'text-red-500'
        ">
