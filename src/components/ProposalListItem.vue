...
<template>
  <div class="text-customMilk bg-customDark dark:bg-[#28334e] rounded text-sm">
        ...
        <tr v-for="(item, index) in proposals?.proposals" :key="index">
          ...
          <td class="w-full">
            <div>
              <RouterLink
                ...
                class="text-main text-base mb-1 block hover:text-grey-100 truncate"
              >
                {{ item?.content?.title || item?.title || metaItem(item?.metadata)?.title }}
              </RouterLink>
              <div
                v-if="item.content"
                class="bg-[#f6f2ff] text-customDark dark:bg-grey-600 dark:text-grey-300 inline-block rounded-full px-2 py-[1px] text-xs mb-1"
              >
          ...
          <td class="w-36">
            <div class="pl-4">
              ...
              <div
                class="truncate col-span-2 md:!col-span-1 text-xs text-grey-500 dark:text-grey-400 text-right md:!flex md:!justify-start"
              >
    ...
    <div class="lg:!hidden">
      ...
      >
        <div
          class="text-customMilk text-base mb-1 flex justify-between hover:text-grey-400"
        >
          <RouterLink
            :to="`/${chain.chainName}/gov/${item?.proposal_id}`"
            class="flex-1 w-0 truncate mr-4 text-customMilk"
            >{{ item?.content?.title || item?.title || metaItem(item?.metadata)?.title }}</RouterLink
          >
          <label
            for="proposal-detail-modal"
            class="text-main text-base hover:text-grey-400 cursor-pointer"
            @click="proposalInfo = item"
          >
            #{{ item?.proposal_id }}</label
          >
        ...
        <div class="grid grid-cols-4 mt-2 mb-2">
          <div class="col-span-2">
            <div
              v-if="item.content"
              class="bg-[#f6f2ff] text-[#c9f31d] dark:bg-grey-600 dark:text-grey-300 inline-block rounded-full px-2 py-[1px] text-xs mb-1"
            >
              {{ showType(item.content['@type']) }}
            </div>
          </div>
          ...
          <div
            class="truncate text-xs text-grey-500 dark:text-grey-400 flex items-center justify-end"
          >
            {{ format.toDay(item.voting_end_time, 'from') }}
          </div>
        </div>
        ...
        <div class="mt-4" v-if="statusMap?.[item?.status] === 'VOTING'">
          <div class="flex justify-between">
            ...
            <label
              for="vote"
              class="btn btn-xs  #sav1 rounded-sm"
              @click="
                dialog.open('vote', {
